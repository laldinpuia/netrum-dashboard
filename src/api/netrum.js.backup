...
export async function exportToPDF({ nodeInfo, miningStatus, claimHistory, tokenData, miningDebug, chartData, dateRange }) {
  ...
  pdf.circle(pageWidth / 2, y + 10, 8, 'F');
  pdf.setTextColor(255, 255, 255);
  pdf.setFontSize(7);
  pdf.text('N', pageWidth / 2, y + 12, { align: 'center' });
  ...
  pdf.text('Generated: ' + formatDateTimeDMY(new Date()), pageWidth - margin, margin + 5, { align: 'right' });
  ...
  if (chartData && chartData.labels && chartData.labels.length > 0) {
    ...
    const chartTitle = dateRange ? 'Claims History (' + dateRange + ')' : 'Claims History Chart';
  ...
  pdf.setFontSize(8);
  pdf.setTextColor(...colors.text);
  pdf.text('Netrum AI Node Dashboard', pageWidth / 2, pageHeight - 8, { align: 'center' });
  pdf.setFontSize(6);
  pdf.setTextColor(...colors.orange);
  pdf.text('Built by D.i.PY | https://dipy.me', pageWidth / 2, pageHeight - 4, { align: 'center' });
