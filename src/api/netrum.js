...
export async function exportToPDF({ nodeInfo, miningStatus, claimHistory, tokenData, miningDebug, chartData, dateRange }) {
  ...
  // HEADER: Logo LEFT of title (side by side layout)
  const logoSize = 14;
  const headerCenterX = pageWidth / 2;
  const titleText = 'Netrum AI';
  const subtitleText = 'Node Dashboard Report';
  
  // Calculate positions for centered logo + title combo
  pdf.setFontSize(18);
  const titleWidth = pdf.getTextWidth(titleText);
  const totalHeaderWidth = logoSize + 4 + titleWidth; // logo + gap + title
  const headerStartX = headerCenterX - totalHeaderWidth / 2;
  ...
  try {
    ...
    if (logoBase64) {
      pdf.addImage(logoBase64, 'PNG', headerStartX, y, logoSize, logoSize);
      logoLoaded = true;
    }
  } catch {
    // Fallback below
  ...
  // Fallback: draw orange circle with N if logo fails
  if (!logoLoaded) {
    pdf.setFillColor(...colors.orange);
    pdf.circle(headerStartX + logoSize / 2, y + logoSize / 2, logoSize / 2, 'F');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(8);
    pdf.text('N', headerStartX + logoSize / 2, y + logoSize / 2 + 2, { align: 'center' });
  }

  // Title "Netrum AI" to the right of logo
  pdf.setFontSize(18);
  pdf.setTextColor(...colors.text);
  pdf.text(titleText, headerStartX + logoSize + 4, y + 6);

  // Subtitle "Node Dashboard Report" below title
  pdf.setFontSize(10);
  pdf.setTextColor(...colors.textMuted);
  pdf.text(subtitleText, headerStartX + logoSize + 4, y + 12);
  ...
  pdf.setFontSize(7);
  pdf.setTextColor(...colors.textMuted);
  pdf.text('Recent Syncs: ' + (node.syncCount || 0), margin + 6, y + 10);
  pdf.text('Active Syncs: ' + activeCount, margin + contentWidth / 3, y + 10);
  pdf.setTextColor(...colors.green);
  pdf.text('Uptime Rate: ' + uptimeRate + '%', margin + contentWidth * 2 / 3, y + 10);
  ...
  if (tokens.totalClaims > 0) {
    ...
    pdf.setFontSize(7);
    pdf.setTextColor(...colors.textMuted);
    pdf.text('Total Claims: ' + tokens.totalClaims, margin + 6, y + 10);
    pdf.setTextColor(...colors.orange);
    pdf.text('Total NPT: ' + tokens.totalNptClaimed?.toFixed(4) + ' NPT', margin + contentWidth / 3, y + 10);
    pdf.setTextColor(...colors.textMuted);
    pdf.text('Avg/Claim: ' + (tokens.totalNptClaimed / tokens.totalClaims).toFixed(4) + ' NPT', margin + contentWidth * 2 / 3, y + 10);
  }

  // FOOTER - Matching CORRECTED.pdf exactly
  ...
  // Thin orange line at top of footer
  pdf.setFillColor(...colors.orange);
  pdf.rect(0, footerY, pageWidth, 0.5, 'F');
  
  // Line 1: "Netrum AI Node Dashboard" - white, centered
  pdf.setFontSize(8);
  pdf.setTextColor(...colors.text);
  ...
  pdf.setTextColor(...colors.textMuted);
  pdf.text('Generated: ' + generatedTimestamp, pageWidth - margin, footerY + 13, { align: 'right' });
